# Reference: https://github.com/MasonEdgar/DCT-Image-Steganography
...
def run():
    ...
    for i in range(CHAR_NUM, CHAR_NUM * 7, CHAR_NUM):
        with open(TEXT_PATH, 'r') as file:
            SECRET_MESSAGE_STRING = file.read(i).rstrip() # rstrip removes spaces at the end of the string
            print(SECRET_MESSAGE_STRING)
       
        for input_file_name in input_files_names:
            COVER_IMAGE_FILEPATH = INPUT_IMAGES_PATH + '/' + input_file_name 
            STEGO_IMAGE_FILEPATH = input_file_name[:-4] + '_Text' + str(i) + '.png'
            ...
            # Write stego image
            cv2.imwrite(OUTPUT_IMAGES_PATH + '/' + STEGO_IMAGE_FILEPATH, final_stego_image)
